...
mapa <- tm_shape(dados) + 
            ...
            tm_credits("Nota: Média do período jan/2012 - dez/2020.",
                       fontface = "italic", size = 0.7,
                       fontfamily = "Garamond", align = "right") +
...
-   Para isso iremos importar o arquivo `data/analise_enade.Rdata` que contém informações sobre o rendimento de discentes universitários no ENADE e estimativas econométricas:
...
-   Vamos agora criar uma tabela em `ASCII`\footnote{Esse formato é útil para sessões interativas no R ou estágios iniciais de estimações, momentos nos quais queremos uma visualização rápida e bem formatada das informações presentes nos dados.} contendo as estatísticas descritivas do `data.frame` *dados_enade* que importamos anteriormente. O código necessário para isso, bem como o resultado obtido podem ser vistos a seguir:
...
```{r temas_basicos_ggplot, echo = FALSE, out.height = "80%", out.width = "85%"}
